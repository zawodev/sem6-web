<template>
  <div class="container">
    ...
    <AppPagination :total-items="readers.length" :current-page="currentPageReaders"
      @update:currentPage="currentPageReaders = $event" :page-size="pageSize" />
  </div>
...
export default {
  ...
  computed: {
    borrowedBooksCount() {
      return this.lendingInfos.filter(info => info.borrower).length;
    },
    paginatedReaders() {
    ...
    paginatedLendingInfos() {
      const borrowed = this.lendingInfos.filter(info => info.borrower);
      const start = (this.currentPageLendings - 1) * this.pageSize;
      const end = start + this.pageSize;
      return borrowed.slice(start, end);
    }
  },
  methods: {
    async fetchReaders() {
